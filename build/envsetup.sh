...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.lineage.device | grep -q "$IRONLOS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $IRONLOS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.lineage.device | grep -q "$IRONLOS_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lineage.device | grep -q "$IRONLOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/ironlos/build/tools/repopick.py $@
}
